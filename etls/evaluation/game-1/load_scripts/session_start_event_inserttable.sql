...
SQL Script(This one): xxxxxxxxxxxxxxxxxxx_inserttable
Purpose: xxxxxxxxxxxxxxxxxxx ETL Beam Pipeline

Input: Date
Expected Output: Delete existing date partitioned data.
                 Populate xxxxxxxxxxxxxxxxxxx data for date partition.
*/
...
INSERT `xxxxxxxxxxxxx.xxxxxx.xxxxxxxxxxxxxxxxxxx` ( event_time_utc, puuid, ip_address, session_id, runtime_platform, realm_id, partition_date, country_code )
    ...
    t AS (
        SELECT
                NET.SAFE_IP_FROM_STRING(player_info.ip_address) & NET.IP_NET_MASK(4,
                                                                                  mask) network_bin,
                mask,
                xxxxxx.to_isoformat(metadata.timestamp) AS event_time_utc,
                ...
                xxxxxx.title(realm_id) AS realm_id,
                DATE(_PARTITIONTIME) AS partition_date
        FROM
            UNNEST(GENERATE_ARRAY(9,32)) mask,
            `xxxxxxxxxxxxx.xxxxxxxxxxxx.session_start`,
        ...
        SELECT
            ...
            CASE
                ...
                END country_code,
        FROM
            `xxxxxxxxxxxxx.xxxxxx.201806_geolite2_city_ipv4_locs` a
                JOIN
            t
            USING
                (network_bin,
